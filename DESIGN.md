...
作る過程で `DESIGN.md` と `screen-spec.json` が実際に何を防ぎ、何を防げなかったかを
[`docs/design-md-report.md`](docs/design-md-report.md) に記録した。

要点だけ:

- 効いた — 部品を増やすたびに宣言の更新が強制された（実際に 5 件の無申告を検出）
- 効かなかった — 意匠の質、何を宣言すべきかの判断、**ドキュメントの主張の真偽**
- 実際に穴があった — 「トークンをバンドルしていない」と書きながら `theme.ts` が
  検査対象外だった期間がある。主張を検査に接続するまで、書いてあることは守られない
